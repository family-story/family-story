...
describe("Add imported med", () => {
  test("add meadia to imported media array", () => {
    let cloud = {
      media_type: 'pic',
      media_ref: 'http://res.cloudinary.com/dgoygxc2r/image/upload/v1521472114/gfj5isonmxqiaelh59pu.jpg'
    }
    let action7 = addUploadedMedia(cloud)
    expect(reducer(initialState2, action7).importedMedia[0].media_ref)
      .toEqual('http://res.cloudinary.com/dgoygxc2r/image/upload/v1521472114/gfj5isonmxqiaelh59pu.jpg')
  })
})

describe("Clear Uploaded Media", () => {
  test("clear uploaded media", () => {
    let action8 = clearImportedMedia()
    expect(reducer(initialState2, action8).importedMedia[0])
      .toEqual(undefined)
  })
})

describe("Set Current story", () => {
  test("get story info", () => {
    let action9 = getStory(2)
    expect(reducer(initialState1, action9).currentStory[0].story_id)
      .toEqual(2)
  })
})

describe("Set current event", () => {
  test("get event info", () => {
    let action10 = getEvent(2)
    expect(reducer(initialState1, action10).currentEvent.event_title)
      .toEqual('Mary Cook converts to "Mormonism"')
  })
})
...
// Court's tests
let homepageState = {
  stories: testStoryList,
  filterString: 'Mary Cook'
}
let stories = []

describe("Home.js Story filter", () => {
  test("filter story by user input", () => {
    stories = homepageState.stories
      .filter(story => {
        let pattern = new RegExp(`${homepageState.filterString}`, 'i')
        if (!homepageState.filterString) {
          return story
...
describe("Home.js Story tags", () => {
  test("mapping through tags", () => {
    let storyTags = stories[0].tags.map(tag => {
      return (
        tag.tag_str
      )
      expect(storyTags.length).toEqual(4)
    })
  })
})
...
function handleAddTag() {
  let tagsArray = storyEditorState.tags.slice()
  tagsArray.push({ tag_str: storyEditorState.tag })
  storyEditorState.tags = tagsArray
}
describe("StoryEditor.js handleAddTag", () => {
  test("adding tags to array", () => {
    handleAddTag()
    expect(storyEditorState.tags.length).toEqual(1)
    expect(storyEditorState.tags[0].tag_str).toEqual('test tag')
  })
...
describe("StoryEditor.js handleRemoveTag", () => {
  test("removing tags from array", () => {
    handleRemoveTag(0)
    expect(storyEditorState.tags.length).toEqual(0)
  })

})

function deleteTestEvent(index) {
  let eventsArr = storyEditorState.events.slice()
  eventsArr.splice(index, 1)
  storyEditorState.events = eventsArr
}
describe("StoryEditor.js deleteTestEvent", () => {
  test("delete an event", () => {
    deleteTestEvent(0)
    expect(storyEditorState.events.length).toEqual(12)
    expect(storyEditorState.events[0].event_id).toEqual(2)
  })
})
...
let eventEditorState = {
 ...
 function addUploadedMediaTest(media_type, media_ref) {    
  let media = {
      media_type,
      media_ref
  }

  let newMedia = eventEditorState.media.slice()
  newMedia.push(media)

  eventEditorState.media = newMedia
}

describe('EventEditor.js addUploadedMediaTest', ()=>{
  test ('it uploads the media', ()=>{
    addUploadedMediaTest('picture', 'test')
    expect(eventEditorState.media[eventEditorState.media.length-1].media_type).toEqual('picture')
  })
})

let eventEditorState2 = {
  event_title: '',
  date: '',
  event_txt: '',
  media: [{
...
describe('EventEditor.js filter', ()=>{
  test('it filters pictures', ()=>{
    let photos = eventEditorState2.media.filter(media => media.media_type === 'pic')
    expect(photos[0].media_type).toEqual('pic')
    expect(photos[1].media_type).toEqual('pic')
    expect(photos[2].media_type).toEqual('pic')
  })
})

describe('EventEditor.js map', ()=>{
  test('it maps the images', ()=>{
    let photos = eventEditorState2.media.filter(media => media.media_type === 'pic')
    let mediaRefs = photos.map(image=>{
      return image.media_ref
    })
    expect(mediaRefs.length).toEqual(3)
  })
})

describe('EventEditor.js filter audio', ()=>{
  test('it makes sure it is audio', ()=>{
    let audio = eventEditorState2.media.filter(media => media.media_type === 'audio')
    expect(audio).toEqual([])
  })
})

describe('EventEditor.js map', ()=>{
  test('it cannot map through the empty array', ()=>{
    let audio = eventEditorState2.media.filter(media => media.media_type === 'audio')
    let audioRefs = audio.map(audio=>{
      return audio
    })
    expect(audio).toEqual([])
  })
